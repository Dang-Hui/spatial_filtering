function [ direction, measure] = t_CBF( R,f,r,c, sig_num )
%'call t_MUSIC'
...
%     end
    p_CBF = [p_CBF,abs(Pcbf)];
end
%====== 寻找峰值 =================================================
comp_num = 7; %峰值和左右两边几个数相比较
direction = [];
peak_val = [];
p_CBF_extended = [p_CBF(end-comp_num+1:end),p_CBF,p_CBF(1:comp_num)];
for i=1:length(p_CBF)
%    i
%   [prod(p_MUSIC_extended(i+comp_num)>p_MUSIC_extended(i:i+comp_num-1)), prod(p_MUSIC_extended(i+comp_num)>p_MUSIC_extended(i+comp_num+1:i+2*comp_num))]
    ...
    if prod(p_CBF_extended(i+comp_num)>p_CBF_extended(i:i+comp_num-1)) && prod(p_CBF_extended(i+comp_num)>p_CBF_extended(i+comp_num+1:i+2*comp_num)) ==1
        direction = [direction,i];
        peak_val = [peak_val,p_CBF(i)];
    end
end
...
% end
% measure = measure;
%text(0.9,-1,['CBF'],'color',[0 0 1])
%text_MVDR = ['CBF角度: ',num2str(sort(direction))];
